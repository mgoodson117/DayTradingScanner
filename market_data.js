...
window.RDT = (function () {
  ...
  // v8.7.3 (2026-06-29) — MA-aware targets & wall guard. Two structure-in-path
  //   fixes after the ON 2026-06-29 miss (featured short while sitting on its
  //   100-day): (A) targets are now capped at an un-cleared 100/200-day in the
  //   path — the target system was previously blind to MAs, so a far target
  //   (e.g. ON's $55) sailed through the 100/200-day and inflated R:R; the cap
  ...
  function scoreStock(d1, m5, spyChangePct, earningsInfo, etCtx, spyContext) {
    ...
    // v8.7.3 — cap targets at an un-cleared major MA in the path. The target
    // derivation above uses only price-action algo lines + raw ATR, so it is blind
    // to the 100/200-day. A target sitting beyond an un-broken major MA overstates
    // the reward; cap it at that MA (the realistic first target) and recompute the
    // ratio. Conservative: only ever TIGHTENS a target, never extends one. The MA
    // must sit on the trade-direction side of the CURRENT price (support below for a
    // short, resistance above for a long). Fixes the structure-blind R:R class that
    // ranked ON's $55 short target straight through its 100/200-day on 2026-06-29.
    if (direction === 'LONG' || direction === 'SHORT') {
      const _majMAs = [d1.sma100, d1.sma200].filter(v => typeof v === 'number' && v > 0);
      const _px = d1.price;
      const _capT = (t) => {
        if (t == null || _px == null) return t;
        let c = t;
        for (const ma of _majMAs) {
          if (direction === 'SHORT') { if (ma < _px && ma > c) c = ma; }      // raise the lower target up to the wall
          else                       { if (ma > _px && ma < c) c = ma; }      // lower the higher target down to the wall
        }
        return parseFloat(c.toFixed(2));
      };
      const _t1c = _capT(rrT1), _t2c = _capT(rrT2);
      if (_t1c !== rrT1 || _t2c !== rrT2) {
        rrT1 = _t1c; rrT2 = _t2c;
        rrT1Source = (rrT1Source || 'target') + ' [capped at major MA]';
        if (rrT1 != null && rrStop != null) {
          if (direction === 'SHORT' && rrEntry < rrStop) {
            const risk = rrStop - rrEntry, reward = rrEntry - rrT1;
            rrRatio = (reward > 0 && risk > 0) ? parseFloat((reward / risk).toFixed(1)) : null;
          } else if (direction === 'LONG' && rrEntry > rrStop) {
            const risk = rrEntry - rrStop, reward = rrT1 - rrEntry;
            rrRatio = (reward > 0 && risk > 0) ? parseFloat((reward / risk).toFixed(1)) : null;
          }
        }
      }
    ...
    let maWallAtPrice = false;       // v8.7.3 — price sitting on a major MA other than a just-confirmed break
    let _confirmedMaLevel = null;    // v8.7.3 — level of the MA whose break/bounce was confirmed (to exclude it from the wall re-check)
      ...
      if (!maConfirmed && Array.isArray(m5 && m5.recentCandles) && m5.recentCandles.length >= 3) {
        ...
        if (ref != null) {
          ...
          if (maConfirmVia === 'M5') { maGovWeight = useCluster
            ...
            _confirmedMaLevel = ref; }
        }
      }
      if (maConfirmed) maConditional = false;
      // v8.7.3 — a confirmed break/bounce of ONE MA does NOT clear the hazard of
      // price sitting on a DIFFERENT major MA (a fresh wall in the path; first
      // touch bounces). Fixes the ON 2026-06-29 miss: a confirmed 50-day break
      // masked price sitting on the 100-day at +0.09×ATR, so the setup was scored
      // as a clean actionable break instead of a short into support.
      if (maConfirmed && maProximity.length) {
        const _nm = maProximity[0];
        const _isMajor = /100|200/.test(_nm.ma);
        const _sameAsConfirmed = _confirmedMaLevel != null
          && Math.abs(_nm.level - _confirmedMaLevel) <= 0.01;
        if (_isMajor && !_sameAsConfirmed && Math.abs(_nm.distAtr) <= _MA_AT_ATR) {
          maWallAtPrice = true;
          maGovWeight = maWeightOf(_nm.ma);   // weight the penalty by the wall MA (100→1.1, 200→1.4)
          maNote += ' ⚠️ WALL: price is also sitting on the ' + _nm.ma + ' $' + _nm.level
            + ' (' + Math.abs(_nm.distAtr) + '×ATR) — a fresh '
            + (direction === 'SHORT' ? 'support' : 'resistance')
            + ' wall in the path. The break is real, but the entry needs a confirmed close '
            + (direction === 'SHORT' ? 'below ' : 'above ') + _nm.level
            + ' first; first touch of the ' + _nm.ma + ' typically bounces.';
        }
      }
    }
    ...
    function _edgeConviction() {
      ...
      // v8.7.3 — sitting on a major MA wall (even with a confirmed break of a
      // different MA) NEGATES the confirmation bonus and docks like a hazard.
      const maAdjBase = maWallAtPrice ? -1.0
                      : maConfirmed ? 1.0
      ...
      // v8.7.3 — nor while sitting on a major MA wall (first-touch bounce risk)
      if (maWallAtPrice && label === 'HIGH') label = 'MEDIUM';
    ...
    if (maWallAtPrice) conviction += ' 🧭 MA-WALL';
    ...
    return {
      ...
      maProximity, maCluster, maState, maConditional, maWallAtPrice, maNote, maConfirmed, maConfirmVia, maGovWeight,
